...
#Grouping for Asset selection
ENEMY_ASSET = np.arange(0,41)
USER_ASSET = np.arange(41,46)
POWERUPS_ASSET = np.arange(46, 50)
NEUTRAL_ASSET = np.arange(50,55)
COLLECTORS_ASSET = np.arange(55,60)

GAMEMODE_ASSET = np.arange(60,63)
ARMODE_ASSET = np.arange(63,66)

GAMEPLAY_ASSET = np.arange(66,73)
...
MIN_GAMEMODE_ASSET = 1

MIN_arMODE_ASSET = 1

MIN_GAMEPLAY_ASSET = 1
...
def trim_variable_postfixes(scope_str):
  ...
  idx = scope_str.find(':')
  #print("trim_variable_postfixes", scope_str[:idx])
...
def get_variable_names(graph, scope):
  ...
  with graph.as_default():
      #print("get_variable name " , [v.name for v in tf.global_variables() if v.name.startswith(scope)])
...
def read_csv():
    ASSET_DICT = []
    with open(r'D:\Program Files\Anaconda3\envs\tf_gpu\Lib\site-packages\magenta\models\rl_tuner\Book1.csv', mode='r') as csv_file:
        csv_reader = csv.reader(csv_file)
        for row in csv_reader:
            ASSET_DICT.append(row)
        return ASSET_DICT
