...
class RLTuner(object):
  ...
  def __init__(self, output_dir,
    ...
    """
    # Make graph.
    print(f"Input size: {input_size} \tNum of Actions: {num_actions}")
    self.graph = tf.Graph()
  ...
  def initialize_internal_models_graph_session(self,
    ...
    print("checkpoint dir ", self.note_rnn_checkpoint_dir)
    with self.graph.as_default():
      ...
      #print("Cell size 1", np.array(self.q_network.state_value))
      ...
      self.target_q_network = note_rnn_loader.NoteRNNLoader(
          ...
          hparams=self.note_rnn_hparams,
          note_rnn_type=self.note_rnn_type)
      #print("Cell size 2", self.target_q_network.cell.state_size)
      ...
      self.reward_rnn = note_rnn_loader.NoteRNNLoader(
          ...
          hparams=self.note_rnn_hparams,
          note_rnn_type=self.note_rnn_type)
      #print("Cell size 3", self.reward_rnn.cell.state_size)
      ...
      if restore_from_checkpoint:
        print("Entering restore from checkpoint")
    ...
    with tf.name_scope('taking_action'):
      ...
      if self.algorithm == 'g':
        ...
        self.action_softmax = tf.nn.softmax(self.action_scores,
                                            name='action_softmax')
        #print("Action",np.array(self.action_softmax))
        self.predicted_actions = tf.one_hot(tf.argmax(self.action_scores,
                                                      dimension=1,
                                                      name='predicted_actions'),
                                            self.num_actions)
       # print("Pred Action", np.array(self.predicted_actions))
  ...
  def train(self, num_steps=100, exploration_period=5000, enable_random=True):
    ...
    #print("\n\nState Value", len(self.q_network.state_value[0]))
    
    tf.logging.info('training initial model...')
    for i in range(num_steps):
      ...
      #print(np.argmax(new_observation))
      self.composition.append(np.argmax(new_observation))
      #print("self.composition ", self.composition)
      self.beat += 1
  ...
  def action(self, observation, exploration_period=0, enable_random=True,
    ...
    #print("sample_next_obs", sample_next_obs)
  ...
  def evaluate_model(self, num_trials=10, sample_next_obs=True):
    ...
    #print("note_rnn_rewards", note_rnn_rewards)
    ...
    for t in range(num_trials):
      ...
      for _ in range(self.num_notes_in_melody):
        ...
        #print("new_observation,reward_scores", new_observation, reward_scores)
  ...
  def collect_reward(self, obs, action, reward_scores):
    ...
    if self.reward_mode == 'scale':
      ...
      print("note_rnn_reward ",note_rnn_reward)
      #print("music_theory_reward_last_n ", self.music_theory_reward_last_n)
      return reward * self.reward_scaler# + note_rnn_reward
...
"""
   ...
   # reward += self.reward_preferred_intervals(action)
    ...
    #self.reward_gamemode(action)
    if reward != prev_reward:
        pass
      #tf.logging.debug('Reward high low unique: %s', reward)
    #Reward function for our implementation
    prev_reward = reward
    reward += self.reward_enemy_composition(action)
    if reward != prev_reward:
      print('\nReward enemy: %s', reward)
    
    prev_reward = reward
    reward += self.reward_user_composition(action)
    if reward != prev_reward:
      print('Reward user: %s', reward)

    prev_reward = reward
    reward += self.reward_powerups_composition(action)
    if reward != prev_reward:
      print('Reward Powerups: %s', reward)
    
    prev_reward = reward
    reward += self.reward_neutral_composition(action)
    if reward != prev_reward:
      print('Reward Neutral: %s', reward)
      
    prev_reward = reward
    reward += self.reward_collectors_composition(action)
    if reward != prev_reward:
      print('Reward collectors: %s', reward)
      
    prev_reward = reward
    reward += self.reward_gamemode_composition(action)
    if reward != prev_reward:
      print('Reward Game mode: %s', reward)
      
    prev_reward = reward
    reward += self.reward_armode_composition(action)
    if reward != prev_reward:
      print('Reward AR Mode: %s', reward)
      
    prev_reward = reward
    reward += self.reward_gameplay_composition(action)
    if reward != prev_reward:
      print('Reward Game play: %s', reward)
      
    prev_reward = reward
    reward += self.supporting_asset(action)
    if reward != prev_reward:
      print('Reward first action game mode: %s', reward)
   # print("Reward", reward)
  ...
  def detect_repeating_notes(self, action_note):
    ...
    for i in range(len(self.composition)-1, -1, -1):
      if self.composition[i] == action_note:
       # print("Repeated happened at ", self.composition[i])
        num_repeated += 1
      else:
        break
  ...
  def generate_music_sequence(self, title='rltuner_sample',
    ...
    lengths = np.full(self.q_network.batch_size, 1, dtype=int)
#    print("self.q_network.batch_size",self.q_network.batch_size)
      ...
      if self.algorithm == 'g':
        ...
        softmax, self.q_network.state_value = self.session.run(
            ...
            {self.q_network.melody_sequence: input_batch,
             self.q_network.initial_state: self.q_network.state_value,
             self.q_network.lengths: lengths})
      #print("State value", self.q_network.state_value)
  ...
  def reward_enemy_composition(self, action, composition = None, reward = 0.5):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
    ...
    #  print("composition ", composition)
      ...
      for x in composition:
          if x in  rl_tuner_ops.ENEMY_ASSET:
              count += 1
      #print("enemy ",count)
      if count < rl_tuner_ops.MIN_ENEMY or count > rl_tuner_ops.MAX_ENEMY:
       #   print("Negative Enemy")
          return -50   
      #print("positive Enemy")
      return reward 
  
    
    
  def reward_user_composition(self, action, composition = None, reward = 0.5):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
      ...
      for x in composition:
          if x in  rl_tuner_ops.USER_ASSET:
              count += 1
      #print("user", count)
      if count < rl_tuner_ops.MIN_USER or count > rl_tuner_ops.MAX_USER:
     #     print("Negative User")
          return -50
    #  print("positive User")
      return reward
  ...
  def reward_powerups_composition(self, action, composition = None, reward = 0.5):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
      ...
      for x in composition:
          if x in  rl_tuner_ops.POWERUPS_ASSET:
              count += 1
   #   print("Powerups", count)
      if count < rl_tuner_ops.MIN_POWERUPS or count > rl_tuner_ops.MAX_POWERUPS:
      #    print("Negative Powerups")
          return -50
     # print("positive Powerups")
      return reward
  
    
    
  def reward_neutral_composition(self, action, composition = None, reward = 0.5):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
      ...
      for x in composition:
          if x in  rl_tuner_ops.NEUTRAL_ASSET:
              count += 1
      #print("Neutral", count)
      if count < rl_tuner_ops.MIN_NEUTRAL or count > rl_tuner_ops.MAX_NEUTRAL:
  ...
  def reward_collectors_composition(self, action, composition = None, reward = 0.5):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
      ...
      for x in composition:
          if x in  rl_tuner_ops.COLLECTORS_ASSET:
              count += 1
   #   print("Collectors", count)
      if count < rl_tuner_ops.MIN_COLLECTORS or count > rl_tuner_ops.MAX_COLLECTORS:
       #   print("Negative Collectors")
          return -50
      #print("positive Collectors")
      return reward
  
    
  def reward_gamemode_composition(self, action, composition = None, reward = 50):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
      ...
      for x in composition:
          if x in  rl_tuner_ops.GAMEMODE_ASSET:
              count += 1
      #print("Game Mode", count)
      if count == rl_tuner_ops.MIN_GAMEMODE_ASSET:
          return reward
      
      return -100
  
    
  def reward_armode_composition(self, action, composition = None, reward = 50):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
      ...
      for x in composition:
          if x in  rl_tuner_ops.ARMODE_ASSET:
              count += 1
      #print("AR Mode", count)
      if count == rl_tuner_ops.MIN_arMODE_ASSET:
          return reward
      
      return -100
    
    
  def reward_gameplay_composition(self, action, composition = None, reward = 50):
      if len(self.composition) + 1 != self.num_notes_in_melody:
          return 0.0
      ...
      for x in composition:
          if x in  rl_tuner_ops.GAMEPLAY_ASSET:
              count += 1
      #print("Game play", count)
      if count == rl_tuner_ops.MIN_GAMEPLAY_ASSET:
       #   print("Game play")
          return reward
      #print("Game play")
      return -100    
  ...
  def reward_gamePLAY(self, action):
      #print("Action ", action)
      if action in rl_tuner_ops.GAMEPLAY_ASSET:
          rl_tuner_ops.GAME_PLAY = list(rl_tuner_ops.GAMEPLAY_ASSET).index(action)
          #print("index ", rl_tuner_ops.GAME_PLAY)
          return True 
    
  def supporting_asset(self, action):
      reward = 0
      present = False
      action_note = np.argmax(action)
      
      ASSET_DICT =  rl_tuner_ops.read_csv()
      if len(self.composition) == 0:
          present = self.reward_gamePLAY(action_note)
      #print("Present", present)
      
      if present or rl_tuner_ops.GAME_PLAY is not None:
          #print("Entering supporting_asset because of action ", action_note )
          reward += 5
          active = ASSET_DICT[action_note][rl_tuner_ops.GAME_PLAY]
         # print("Active", active)
         # print("ASSET_DICT ", ASSET_DICT[action_note])
          if active == '1':
              reward += 10
      return reward
